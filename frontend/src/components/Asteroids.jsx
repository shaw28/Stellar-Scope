...
export default function Asteroids() {
  ...
  const fetchAsteroids = () => {
    setLoading(true);
    axios.get('https://stellarscope-sm5o.onrender.com/api/asteroids', {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto font-orbitron">
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-6">
        ...
        <button onClick={fetchAsteroids} className="bg-yellow-400 hover:bg-yellow-300 text-black px-4 py-2 rounded font-bold">Fetch</button>
      </div>
      ...
      <div className="grid gap-4">
        {asteroids.map(ast => (
          <div key={ast.id} className="bg-gray-800 p-4 rounded text-white shadow">
            ...
            <p>📏 Diameter: {ast.estimated_diameter.meters.estimated_diameter_max.toFixed(1)} m</p>
            <p>🛰 Velocity: {parseFloat(ast.close_approach_data[0]?.relative_velocity.kilometers_per_hour).toFixed(0)} km/h</p>
            <p>📍 Miss Distance: {parseFloat(ast.close_approach_data[0]?.miss_distance.kilometers).toFixed(0)} km</p>
          </div>
        ))}
      </div>
    </div>
